...
class BookingControllerTest extends BaseIntegTest {
    ...
    @Test
    void getBookingsOfOwner_stateUNKNOWN_returnsBadRequest() {
        User owner = saveAndGetUser("booker@email.com");
        webTestClient.get().uri(BOOKINGS_BASE_URL + "/owner?state=UNKNOWN&from=0&size=10")
    ...
    @Test
    void getBookingsOfUser_stateUNKNOWN_returnsBadRequest() {
        User booker = saveAndGetUser("booker@email.com");
        webTestClient.get().uri(BOOKINGS_BASE_URL + "?state=UNKNOWN&from=0&size=10")
                .header(USER_HEADER, booker.getId().toString())
                .exchange()
                .expectStatus().isBadRequest();
    }
